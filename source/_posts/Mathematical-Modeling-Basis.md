...
**摘要**：概述论文问题背景，直接给出我们所建立的数学模型作为摘要的大纲，后续都围绕着我们所构建的模型来写。针对所建立的模型，阐述模型的创新点、所建立的模型类型、求解方法、求解结果、结论等。模型、算法、结果要一目了然。对重点内容加粗凸显，最后总结敏感性分析。摘要中需要加粗的文本包括——模型名称、因素名称、求解方法、关键数值结论等，加粗的内容占比要合理。摘要的篇幅限制在第一页之内，不宜过长导致翻页。
